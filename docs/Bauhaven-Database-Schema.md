...
| "Does this applicant already have an open application?" | `idx_applications_one_open_per_applicant` (partial unique — `submitted`/`confirmed` only) |
...
**013_admin_create_account.sql is not a policy fix — it's the one place the platform deliberately steps outside RLS, so the reasoning belongs here.** Creating an account means writing `auth.users`, which no RLS policy can grant and no anon key should ever be able to do. The function is `security definer` and checks `auth_is_admin()` as its first statement, since a definer function without that check is an open door running as the owner. Its defining property, though, is what it *won't* do: it refuses an email that already exists and never updates one. `supabase/seed/002_seed_accounts.sql` resets the password on re-run, which is right for a script a DBA iterates on and catastrophic once the same behaviour is reachable from a web session — any Admin could overwrite another Admin's password and sign in as them, with an audit trail that reads like onboarding. The body also carries the two things a working sign-in needs and an inspection won't reveal: the four token columns written as `''` rather than NULL (GoTrue reads them into non-nullable Go strings, so a NULL row fails to load *before* the password is checked) and the `auth.identities` row without which the account exists, appears in the dashboard, and cannot log in. Both cost a debugging round to find the first time — see 002 and 004.

**A sixth was caught while building Academy's self-signup — `012_academy_signup_applications.sql`.** `applications_select` was `using (auth_is_admin_or_staff())`, which is right for a form the public website posts anonymously and wrong the moment the applicant has an account: they could file an application and then not read it, so the screen telling them where it stands would have had nothing to show. `012` adds `applicant_id` and an own-row arm, `applicant_id = auth.uid()`, exposing their row and nothing else. The same migration adds `idx_applications_one_open_per_applicant`, a **partial** unique index over `status in ('submitted', 'confirmed')` — the database is what stops a double submission, not the form, and scoping it to the open statuses is what still allows re-applying after a decline while leaving anonymous applications (`applicant_id is null`) unaffected.
