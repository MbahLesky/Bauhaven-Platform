...
- Issue reports route to Staff **by category** (e.g. asset problems → Assets-permission staff), not all through Admin first. *(**The schema does not implement this, and the wording overstates what exists.** Checked while building Academy's submission screen: the word "category" appears in exactly two places in the whole schema — the `issue_reports.category` column (`text not null default 'general'`, **no check constraint**) and the composite index `idx_issue_reports_status_category`. There is no per-category staff assignment table, and nothing links a category to `user_roles.staff_sub_role` or to a permission. `issue_reports_select` and `issue_reports_update` both gate on plain `auth_is_admin_or_staff()` with **no category arm**, so every Staff member and every Admin can read and resolve every report regardless of category. "Routed by category" today means the category is a **filterable, displayable label** — the index exists so a triage screen can sort and filter on it — not a destination. Real routing would need either a category→staff mapping table or a category arm in the policies; neither exists, and neither was built speculatively.)*
...
- **Approval quorum depends on who's requesting:** a User's request needs one Staff approval; a Staff's request needs one Admin approval; an **Admin's own request needs approval from every other Admin** (unanimous, not just one) — this needs a `RequestApproval` join table (one row per required approver) rather than a single `approver_id` field on `Request`, since the Admin case can require more than one signer. **If there's only one Admin total, the request auto-approves** (zero other Admins required = quorum trivially met). *(Status as built: **none of this is implemented, and RLS currently prevents it.** `bauhaven-academy-web` submits requests and Staff/Admin can read them, but `requests` has no UPDATE policy — so no row can leave `'pending'` — and `request_approvals` has no INSERT policy, so no approver row can be created. A migration has to land before an approval screen can work, and its `with check` clause is where this quorum rule gets encoded. Two things it needs that don't exist yet: a way to know **which** Staff member approves a given User's request (`users` has no supervisor link), and a decision on whether approver rows are created at submission or lazily at first review. See the Academy Feature Spec §7, "Requests".)*
