...
- **next-intl is in the Tech Stack doc but has never been set up, in either app — recommended as the next dedicated task.** `Bauhaven-Tech-Stack.md` lists it for UI chrome, noting that bilingual *content* lives in `_en`/`_fr` columns. **The content half is done.** Admin-web's Content Editor writes both languages for pages and portfolio entries; Academy's testimony form routes a student's words to `content_en` or `content_fr`; and Academy's Profile screen now persists `users.preferred_language` through a working EN/FR toggle that those features already consume. **The chrome half is untouched:** every label, button, error message and empty state across Admin-web and Academy-web is a hard-coded English string, including the ones on the screen holding that toggle. A student can set their language to French and see an entirely English interface.

  This has now been flagged three times while building — Content Editor (content), Testimony (chrome), Profile (the toggle) — and the third pass did the part that could be done honestly in one screen rather than deferring again. What remains genuinely is its own piece of work: it spans both apps, touches every screen already shipped, and needs decisions about routing (`/fr/...` vs. cookie), where translation files live, and whether Admin-native and Academy-native share the message catalogue. **The recommendation is to schedule it as its own task before more screens are built**, since every screen added meanwhile is more strings to extract — Academy-web alone went from three routes to ten while this sat open.
