...
4. The migrations are already tested against a live Postgres instance with seeded accounts — see `docs/Bauhaven-Database-Schema.md` for what was tested and the four real bugs caught and fixed along the way (an RLS recursion issue in `002`, an application-approval gap in `003`, an ungradeable `submissions` table in `004`, and an unapprovable `finance_records` in `005`)
